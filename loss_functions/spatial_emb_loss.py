...
from loss_functions.lovasz_losses import lovasz_hinge, lovasz_softmax


class SpatialEmbLoss(nn.Module):

    def __init__(self, to_center=True, n_sigma=1, foreground_weight=1, num_classes=1):
        super().__init__()
    ...
    def forward(self, prediction, instances, labels, w_inst=1, w_var=10, w_seed=1, iou=False, iou_meter=None):
        ...
        for b in range(0, batch_size):
            ...
            seed_map = torch.sigmoid(
                    prediction[b, 2+self.n_sigma:2+self.n_sigma+self.num_classes]
                )  # C x h x w
            ...
            for id in instance_ids:
                ...
                in_mask = instance.eq(id) 
                # 1 x h x w
                class_label = label[in_mask].unique()
                ...
                instance_loss = instance_loss + lovasz_hinge(dist*2-1, in_mask.long())

                # seed loss
                seed_loss += self.foreground_weight * torch.sum(
                    torch.pow(seed_map[class_label, in_mask.squeeze(0)] - dist[in_mask].detach(), 2))
