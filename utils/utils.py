...
class Cluster:
    ...
    def __init__(self, ):
        xm = torch.linspace(0, 2, 1024).view(1, 1, -1).expand(1, 1024, 1024)
    ...
    def cluster_with_gt(self, prediction, instance, n_sigma=1,):
        ...
        instance_map = torch.zeros(height, width).byte().to(device)
    ...
    def cluster(self, prediction, class_labels, n_sigma=1, threshold=0.5, num_classes=1): #change
        ...
        spatial_emb = torch.tanh(prediction[0:2]) + xym_s  # 2 x h x w
        sigma = prediction[2:2+n_sigma]  # n_sigma x h x w
        seed_map = torch.sigmoid(prediction[2+n_sigma:2+n_sigma + num_classes])  # C x h x w #change
       
        instance_map = torch.zeros(height, width).byte()
        instances = []
        print(f'prediction size {prediction.size()}, class_labels size {class_labels.size()}')
        count = 1
        print(f'seed map size {seed_map[1].size()}')
        for class_label in class_labels.unique():
            mask = seed_map[class_label.item()-1] > 0.5
            if mask.sum() > 128:

                spatial_emb_masked = spatial_emb[mask.expand_as(spatial_emb)].view(2, -1)
                sigma_masked = sigma[mask.expand_as(sigma)].view(n_sigma, -1)
                seed_map_masked = seed_map[class_label.item()-1, mask].view(1, -1)

                unclustered = torch.ones(mask.sum()).byte().to(device)
                instance_map_masked = torch.zeros(mask.sum()).byte().to(device)
